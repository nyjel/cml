...
melEnv=Adsr(attack=0.5, decay=0.5, sustain=0.5, release=0.5, mul=1, dur=2)
...
def melFromRow():
    ...
    melrow = random.random(16,)
    print 'melcount', melcount, 'melrow', melrow
    block=1
    note=int(math.floor(16* melrow[melcount]))
...
def pat():
  ...
  if count >= transient:
      ...
      env = Adsr(attack=1, decay=.3, sustain=.5, release=.3, dur=3, mul=subamp)
      a.mul=env
      f.q=12+ h*5
